...
export function ExpandableBentoCard({
  ...
  // 1: flipping to back
  // 2: expanded
  // 3: morphing back to small
  const [step, setStep] = useState<0 | 1 | 2 | 3>(0);

  // When step 3 is set (closing), wait for layout animation to drop back to small card, then flip to front
  useEffect(() => {
    if (step === 3) {
      const timer = setTimeout(() => {
        setStep(1); // switch back to the 'back' card before flipping
        setTimeout(() => setStep(0), 50); // then start flip to front
      }, 400); // Wait for the AnimatePresence layoutId exit to roughly finish
        ...
        {step !== 2 && (
          ...
          >
            ...
            >
              ...
              {/* Back Side (Blank color just to look like a card back) */}
              <div 
